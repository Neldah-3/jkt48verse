...
import { db } from "@/db";
import { loginLogs, reports, users, moderationLogs } from "@/db/schema";
import { counts } from "@/lib/data";
import { getLiveNow } from "@/lib/live";
import { staffCount } from "@/lib/credentials";
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ s?: string }> }) {
  ...
  const [[pend], c, live, reps, us, logs, mods, sc] = await Promise.all([
    db.select({ n: count() }).from(reports).where(eq(reports.status, "pending")), counts(), getLiveNow(),
    db.select().from(reports).where(s === "all" ? undefined : eq(reports.status, s)).orderBy(desc(reports.createdAt)).limit(30),
    db.select().from(users).orderBy(desc(users.createdAt)).limit(30),
    db.select().from(loginLogs).where(and(eq(loginLogs.kind, "staff"), gte(loginLogs.createdAt, new Date(Date.now() - 7 * 86400_000)))).orderBy(desc(loginLogs.createdAt)).limit(10),
    db.select().from(moderationLogs).orderBy(desc(moderationLogs.createdAt)).limit(10), Promise.resolve(staffCount()),
  ]);
  ...
  return (
    <>
      <PageHead title="Admin Panel" sub={`Masuk sebagai ${v.username} (${v.staffId}) · ${sc.admins} Admin · ${sc.moderators} Moderator terdaftar di kredensial server`} />
      <div className="grid12 mb-3.5">
        <div className="c3 card w stat"><div className="lbl"><Icon name="flag" size={13} /> Report Pending</div><div className="num" style={{ color: "var(--warn)" }}>{pend.n}</div></div>
      ...
      <div className="grid12">
        <div className="c8 flex flex-col gap-3.5">
          ...
          <div className="card w">
            <div className="w-head"><h3>Manajemen Akun</h3><span className="muted text-[11px]">Admin/Moderator via file kredensial (read-only)</span></div>
            <div className="overflow-x-auto"><table className="tb min-w-[560px]"><thead><tr><th>User</th><th>Role</th><th>Poin</th><th>Status</th><th>Bergabung</th><th></th></tr></thead><tbody>
              {us.map((u) => { const blocked = u.blockedUntil && u.blockedUntil.getTime() > Date.now(); const muted = u.mutedUntil && u.mutedUntil.getTime() > Date.now(); return (<tr key={u.id}><td><span className="inline-flex items-center gap-2"><Avatar name={u.username} size={22} seed={u.avatarSeed} />{u.username}</span></td><td><span className="tag t-gray">{u.role}</span></td><td className="tabular">{u.points}</td><td>{blocked ? <span className="tag t-red">Blokir</span> : muted ? <span className="tag t-warn">Mute</span> : <span className="tag t-ok">Aktif</span>}</td><td className="muted whitespace-nowrap">{fmtDateTime(u.createdAt)}</td><td><SanctionButton userId={u.id} username={u.username} role={u.role} /></td></tr>); })}
            </tbody></table></div>
          </div>
        </div>
        <div className="c4 flex flex-col gap-3.5">
          ...
          <div className="card w"><div className="w-head"><h3>Login Staff (7 hari)</h3></div>{logs.map((l) => (<div key={l.id} className="row items-center text-[12px]"><span className="flex-1">{l.username}</span>{l.success ? <span className="tag t-ok">ok</span> : <span className="tag t-red">gagal</span>}<span className="muted text-[11px]">{fmtDateTime(l.createdAt)}</span></div>))}</div>
        </div>
      </div>
      <p className="muted text-[11px] mt-4">Moderator: maks ban 30 hari, permanen butuh approval Admin · hanya 3 ADMIN & 10 MODERATOR via file kredensial server (tidak dapat diubah lewat web).</p>
    </>
  );
}
