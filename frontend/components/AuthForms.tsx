...
import { forgotPasswordAction, loginAction, registerAction, resendOtpAction, resendResetOtpAction, resetPasswordOtpAction, staffLoginAction, verifyOtpAction, type ActionResult } from "@/app/actions";
...
export function LoginForm({ next }: { next?: string }) {
  ...
  return (
    <form action={action} className="flex flex-col gap-3">
      ...
      <p className="muted text-[12.5px] text-center"><Link href="/auth/forgot-password" className="link">Lupa password?</Link></p>
...
export function ForgotPasswordForm({ email }: { email?: string }) {
  const [state, action] = useFormState(forgotPasswordAction, init);
  return (
    <form action={action} className="flex flex-col gap-3">
      <div><label className="label">Email akun kamu</label><input name="email" type="email" className="input" required defaultValue={email} autoComplete="email" /></div>
      {!state.ok && <p className="text-primary text-[12.5px]">{state.error}</p>}
      <SubmitButton label="Kirim Kode OTP" pendingLabel="Mengirim…" />
      <p className="muted text-[11px]">Kami akan mengirim kode OTP 6 digit ke email-mu (berlaku 10 menit). Demi keamanan, kami tidak memberi tahu apakah email terdaftar atau tidak.</p>
      <p className="muted text-[12.5px] text-center"><Link href="/auth/login" className="link">‹ Kembali ke login</Link></p>
    </form>
  );
}

export function ResetPasswordForm({ email, devCode }: { email?: string; devCode?: string }) {
  ...
  return (
    <form action={action} className="flex flex-col gap-3">
      <div><label className="label">Email</label><input name="email" type="email" className="input" required defaultValue={email} autoComplete="email" /></div>
      <div><label className="label">Kode OTP (6 digit)</label><input name="code" className="input" required pattern="[0-9]{6}" maxLength={6} inputMode="numeric" autoComplete="one-time-code" /></div>
      <div><label className="label">Password baru (min. 8 karakter)</label><input name="newPassword" type="password" className="input" required minLength={8} maxLength={64} autoComplete="new-password" /></div>
      <div><label className="label">Ulangi password baru</label><input name="confirmPassword" type="password" className="input" required minLength={8} maxLength={64} autoComplete="new-password" /></div>
      {!state.ok && <p className="text-primary text-[12.5px]">{state.error}</p>}
      {devCode && <p className="text-[12.5px]" style={{ color: "var(--warn)" }}>Mode dev (RESEND belum diatur) — kode OTP kamu: <b>{devCode}</b></p>}
      ...
      {resent && <p className="muted text-[12px]">{resent}</p>}
      <p className="muted text-[11px]">Setelah reset berhasil, semua sesi login lama otomatis keluar dan kamu perlu login ulang dengan password baru.</p>
