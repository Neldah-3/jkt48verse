...
from src.db import Database, upsert_data
...
class BaseScraper(ABC):
    ...
    Template: run() defines the fixed algorithm:
        ...
        3. Save to JSON
        4. Optionally sync to PostgreSQL (Supabase)

    Subclasses only need to define:
        - name: display name
        - output_file: JSON output path
        - collection_name: PostgreSQL table
    ...
    def sync(self, data):
        """Sync data to PostgreSQL (Supabase)."""
        items = self._get_items(data)
        print(f"\n🔌 Syncing {self.name.lower()} to PostgreSQL...")
        db = Database()
        if db.connect():
            stats = upsert_data(db, self.collection_name, items)
            print(
...
class ScheduleScraper(BaseScraper):
    ...
    def sync(self, data):
        ...
        if isinstance(data, dict) and data.get("members"):
            print("🔌 Syncing involved members to PostgreSQL...")
            db = Database()
            if db.connect():
                m_stats = upsert_data(db, "members", data["members"])
                print(
...
class HistoricalScheduleScraper(BaseScraper):
    ...
    def sync(self, data):
        """Sync events and involved members."""
        print(f"\n🔌 Syncing {self.year} events to PostgreSQL...")
        db = Database()
        if db.connect():
            stats = upsert_data(db, self.collection_name, self._get_items(data))
            print(
...
if __name__ == "__main__":
  ...
  python jkt48scraper.py --news --sync         # Fetch news and sync to PostgreSQL
  python jkt48scraper.py --members --sync      # Fetch members and sync to PostgreSQL
  python jkt48scraper.py --schedule --sync     # Fetch schedule and sync to PostgreSQL
        """,
    )
    ...
    parser.add_argument(
        "--sync", action="store_true", help="Sync fetched data to PostgreSQL (Supabase)"
    )
