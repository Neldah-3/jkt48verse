...
class ConcertsService:
    def __init__(self, repository: ConcertsRepository, config: Settings):
    ...
    async def create_concert(self, data: CreateConcert) -> ConcertResponse:
        try:
            concert = await self.repository.insert_concert(data)
            return self._map_to_response(concert.to_dict())
    ...
    async def update_concert(self, concert_id: str, data: UpdateConcert) -> ConcertResponse:
